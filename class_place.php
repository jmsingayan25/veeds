	...
	*/
	class classPlaceID{
		...
		public function setPlaceId($setplaceid){
			...
			// sleep(2);
			$hostname = "https://maps.googleapis.com/maps/api/place/details/json?placeid=".str_replace(" ", "+", $setplaceid)."&key=AIzaSyBLYHLLfWNbEdHvRJKdArowB8tjmFb1xkE";
	...
	// echo $hostname;
		...
		public function setCoordinatesCity($coordinates,$city){
			...
			// sleep(2);
			$hostname = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=".$coordinates."&rankby=distance&keyword=".str_replace(" ", "+", $city)."&key=AIzaSyBLYHLLfWNbEdHvRJKdArowB8tjmFb1xkE";
	// echo $hostname;
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $hostname);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
