<?php
	...
	*/	
				...
				// $search6['select'] = "word";
				// $search6['table'] = "veeds_dictionary";

				// $result6 = jp_get($search6);

				// while ($row6 = mysqli_fetch_assoc($result6)) {
				// 	$row6['word'] = str_replace("\n", "", $row6['word']);
				// 	$list_of_words[] = $row6['word'];
				// }
			...
			$search4['select'] = "h.user_id, firstname, lastname, username, personal_information, profile_pic, h.place_id, location_id";	
			$search4['table'] = "veeds_users u, veeds_establishment e, veeds_users_visit_history h";	
			$search4['where'] = "u.user_id = h.user_id 
									AND h.place_id = e.place_id
									AND h.user_id NOT IN (SELECT user_id_follow FROM veeds_users_follow
															WHERE user_id = '".$_POST['user_id']."')
									AND date_visit IN (SELECT MAX(date_visit)
														FROM veeds_users_visit_history
														GROUP BY user_id)";	
			$search4['filters'] = "GROUP BY u.user_id";
