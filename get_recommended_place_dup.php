<?php
	...
	$_POST['user_id'] = "183";
	$_POST['coordinates'] = "14.585016,121.056145";
	$_POST['city'] = "Mandaluyong";
...
*/		
		$search2['select'] = "h.place_id, location_id, e.coordinates, h.user_id, h.video_id, video_thumb, date_upload, date_expiry";
		...
		$search2['where'] = "h.place_id = e.place_id
								...
								AND date_visit IN (SELECT MAX(date_visit) 
															FROM veeds_users_visit_history
              												WHERE user_id IN (".$users.") 
              												GROUP BY place_id)";
    	$search2['filters'] = "GROUP BY h.place_id ORDER BY date_upload DESC LIMIT 1";
...
*/
		$search3['select'] = "h.place_id, location_id, e.coordinates, h.user_id, h.video_id, video_thumb, date_upload, date_expiry";
		...
		$search3['where'] = "h.place_id = e.place_id
								...
								AND date_visit IN (SELECT MAX(date_visit) 
													FROM veeds_users_visit_history
      												WHERE user_id IN (".$users.") 
      												GROUP BY place_id)";
		$search3['filters'] = "GROUP BY h.place_id ORDER BY date_upload DESC LIMIT 1";
