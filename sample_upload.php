...
<?php
	...
	if(isset($_POST['submit']) && isset($_FILES)){
		...
		if((!isset($_GET['landscape']) && !empty($filename)) || (isset($_GET['landscape']) && !empty($filename) && isset($filename3))){
			...
			if(isset($_GET['coordinates']) && isset($_GET['location']) && isset($_GET['place_id'])){
				...
				if($visit_count['visit_count'] < 2){ //check if user visited the place for the first time
					...
					if(jp_count($search5) == 0){ //check if place_id not exist on the table
						...
						$new_count = array();
						$new_count['total_count'] = $row4['total_count'] + 1;

						$visit['data'] = $new_count;
			...
			$result = array('reply' => '1', 'video' => $filename, 'place_id' => $_GET['place_id'], 'title' => $_GET['video_name'], 'description' => $_GET['description'], 'user_id' => $_GET['user_id'], 'date_upload' => $_GET['date_upload'], 'location' => $_GET['location'], 'thumb' => $filename2, 'video_id' => $video_id, 'list' => $list, 'files' => $_FILES);
		}else
