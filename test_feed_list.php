<?php
	...
	if(isset($_POST['user_id'])){
		...
		$placeIdDetail1 = new classPlaceID;
		$placeIdDetail2 = new classPlaceID;
		$placeIdDetail3 = new classPlaceID;
		$placeIdDetail4 = new classPlaceID;
		$placeIdDetail5 = new classPlaceID;

		$search['select'] = "DISTINCT a.video_id, a.video_name, a.description, a.video_id, b.firstname, a.video_thumb, b.lastname, a.date_upload, a.view_count, a.location, b.profile_pic, a.video_length, b.username, a.user_id as uploader_user_id, a.coordinates, a.landscape_file, a.timestamp, a.place_id";
		...
		$search['where'] = "a.user_id = b.user_id AND a.user_id IN (".$in.") 
							AND a.user_id != '".$_POST['logged_id']."' AND b.disabled = 0"; 
		// $search['where'] = "a.user_id = b.user_id AND a.user_id IN (".$in.") AND b.disabled = 0
		...
		$search['filters'] = "GROUP BY a.place_id ORDER BY a.date_upload DESC LIMIT ".$start.", 100";
		// $search['filters'] = "ORDER BY a.date_upload DESC LIMIT 1";
		//echo implode(" ", $search);
		...
		if(jp_count($search3) > 0){	// display place id if no posts associated to the city existed in database
			...
			while ($row3 = mysqli_fetch_assoc($return3)) {
				...
				if(!in_array($row3['location_id'],$location['location'])){
					...
					// sleep(2);
					// $placeIdDetail->setPlaceId($row3['place_id']);
					// $placeName = $placeIdDetail->getPlaceName();
			...
			if(jp_count($search15) > 0){	
				...
				while ($row15 = mysqli_fetch_assoc($result15)) {
					...
					if(!in_array($row15['location_id'],$location['location'])){
						$location['location'][] = $row15['location_id'];

						// sleep(2);
						// $placeIdDetail->setPlaceId($row15['place_id']);
						// $placeName = $placeIdDetail->getPlaceName();
			...
			// get place id from google
				...
				if(jp_count($search16) > 0){
					...
					while ($row16 = mysqli_fetch_assoc($result16)) {
						if(!in_array($row16['location_id'],$location['location'])){
							$location['location'][] = $row16['location_id'];
					...
					if($cityPlaceID != ''){
						...
						if(jp_count($search17) > 0){
							...
							while ($row17 = mysqli_fetch_assoc($result17)) {
								if(!in_array($row17['location_id'],$location['location'])){
									$location['location'][] = $row17['location_id'];
              	...
              	if(jp_count($search4) > 0){
              		...
              		while ($row4 = mysqli_fetch_assoc($result4)) {
              			...
              			if(!in_array($row4['location_id'],$location['location'])){
							...
							// $placeIdDetail->setPlaceId($row4['place_id']);
							// $placeName = $placeIdDetail->getPlaceName();
...
*/	
							...
							if(jp_count($search7) > 0){
					 			...
					 			while ($row7 = mysqli_fetch_assoc($result7)) {
					 				...
					 				if(!in_array($row7['location_id'],$location['location'])){
										...
										// $placeIdDetail->setPlaceId($row7['place_id']);
										// $placeName = $placeIdDetail->getPlaceName();
					...
					if(jp_count($search8) > 0){
			 			...
			 			while ($row8 = mysqli_fetch_assoc($result8)) {
			 				...
			 				if(!in_array($row8['location_id'],$location['location'])){
								...
								// $placeIdDetail->setPlaceId($row8['place_id']);
								// $placeName = $placeIdDetail->getPlaceName();
		...
		if(jp_count($search9) > 0){
			...
			while($row9 = mysqli_fetch_assoc($result9)){
				...
				$search10['select'] = "date_upload";
				$search10['table'] = "veeds_videos";
				$search10['where'] = "user_id = '".$row9['user_id']."' 
										AND date_upload IN (SELECT MAX(date_upload) 
															FROM veeds_videos 
															WHERE user_id = '".$row9['user_id']."')"; 
